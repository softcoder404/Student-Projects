...
def computeWindow():
    ...
    voltage_label = Label(top,text="Voltage (V) :  460")
    phase_label = Label(top,text="Phases :  3")
    frequency_label = Label(top,text="Frequency (Hz) :  60")
    ...
    friction_and_windage_label = Label(top,text="Friction and Windage (kw) :  0.074")
    stray_load_label = Label(top,text="Stray-Load Loss (kw) :  0.097")
    ...
    R_2 = float(result["r2"])
    R_1 = float(result['r1'])
    final_result['i_1'] = round(I_1,2)
    final_result['i_2'] = round(I_2,2)
...
def finalCalculation():
    ...
    r2_act_cal = round(r2_act_cal,3)
    sec_input = ((float(result['phase']) * (final_result['i_2'])**2) * (r2_act_cal/float(result['assume_slip'])))/1000
    sec_loss = (float(result['phase']) * final_result['i_2'] *final_result['i_2'] * r2_act_cal)/1000
    op = (sec_input  - (sec_loss + float(result['friction and windage']) + float(result['stray load'])))
    r1_act_cal = final_result['r1'] * final_result['r2_base']
    pri_loss = (r1_act_cal * final_result['i_1'] * final_result['i_1'] * float(result['phase'])) / 1000
    total_loss = pri_loss + sec_loss + 0.168 + float(result['friction and windage']) + float(result['stray load'])
    ...
    torque = (float(result['horse power']) * 5252) / speed
    #result label
    ...
    output_power_res = Label(top,text="output (kw)= "+str(op))
    ...
    button = Button(top,text= "Draw Graph",command=finalCalculation)
...
def extraSlips():
    top = Toplevel()
    top.title("Extra Slips and Tables Generation")
    #Create a label for each slips
    title_label = Label(top, text="Please Insert All Slips Values")
    slip1_label = Label(top, text= "Slip 1 : ")
    slip2_label = Label(top, text= "Slip 2 : ")
    slip3_label = Label(top, text= "Slip 3 : ")
    slip4_label = Label(top, text= "Slip 4 : ")
    slip5_label = Label(top, text= "Slip 5 : ")
    slip6_label = Label(top, text= "Slip 6 : ")
    #Create an entry for each slips
    slip1_entry = Entry(top, widget=20,borderwidth=2)
    slip2_entry = Entry(top, widget=20,borderwidth=2)
    slip3_entry = Entry(top, widget=20,borderwidth=2)
    slip4_entry = Entry(top, widget=20,borderwidth=2)
    slip5_entry = Entry(top, widget=20,borderwidth=2)
    slip6_entry = Entry(top, widget=20,borderwidth=2)
    #arranging the entry and label in column 1
    slip1_label.grid(row=0,column=0)
